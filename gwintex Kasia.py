# -*- coding: utf-8 -*-
"""
Created on Mon May  2 10:12:19 2016

@author: kasia
"""

#cena zestawu powinna być ustalona w relacji do kosztu bezczynnosci tak żeby nie wyszło w optimum 6 zestawow - Kasia
...
t_avg_L = round(np.mean(t_skum_L), 2)
t_avg_G = round(np.mean(t_skum_G), 2)

#odchylenie standardowe
t_sd_L = round(np.std(t_skum_L), 2)
t_sd_G = round(np.std(t_skum_G), 2)
...
#print(t_skum_G)
#
#print("Średni czas bezczynności w układzie liniowym: " + str(t_avg_L) + " z odchyleniem standardowym: " + str(t_sd_L))
#print("Średni czas bezczynności w układzie gniazdowym: " + str(t_avg_G) + " z odchyleniem standardowym: " + str(t_sd_G))
#
...
for opt_l_zest in range(1, 8):
    opt_l_zest_L.append((np.mean(symulacja(l_symul, l_maszyn, opt_l_zest, horyzont, sr_t_awaria, 
                         sr_t_naprawy, "L")), opt_l_zest))
    opt_l_zest_G.append((np.mean(symulacja(l_symul, l_maszyn, opt_l_zest, horyzont, sr_t_awaria, 
                         sr_t_naprawy, "G")), opt_l_zest))

min_avg_t_L = min([i[0] for i in opt_l_zest_L])
min_avg_t_G = min([i[0] for i in opt_l_zest_G])

opt_zest_L = [i for i in opt_l_zest_L if i[0] == min_avg_t_L]
opt_zest_G = [i for i in opt_l_zest_G if i[0] == min_avg_t_G]
                     
print("Optymalna liczba zestawów w układzie liniowym: " + str(opt_zest_L[0][1]) + ", średni czas bezczynności: " + str(opt_zest_L[0][0]))
print("Optymalna liczba zestawów w układzie gniazdowym: " + str(opt_zest_G[0][1]) + ", średni czas bezczynności: " + str(opt_zest_G[0][0]))
